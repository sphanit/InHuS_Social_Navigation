...
LogManager::LogManager()
{
	...
	sub_h2_pose_vel_ = nh_.subscribe("known/human2_pose_vel", 100, &LogManager::h2PoseVelCallback, this);
...
void LogManager::h2PoseVelCallback(const inhus::PoseVel::ConstPtr& msg)
{
	if(ros::Time::now().toSec() != 0.0)
		log_file_inhus_poses_ << ros::Time::now() << " : H2 " << msg->pose.x << " " << msg->pose.y  << " " << msg->pose.theta << endl;
		log_file_inhus_ << ros::Time::now() << " : LOG VEL_H2 " << std::to_string(sqrt(pow(msg->vel.linear.x,2) + pow(msg->vel.linear.y,2))) << endl;
}
